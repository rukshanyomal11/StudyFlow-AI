...
function createErrorResponse(error) {
  console.error('Mentor content item API error:', error);
  ...
  if (error instanceof Error) {
    ...
    if (
      ...
      error.message === `Visibility must be one of: ${MATERIAL_VISIBILITY.join(', ')}` ||
      error.message === 'Status must be one of: Published, Draft, Archived'
    ) {
...
async function ensureSubjectAccess(subjectId, mentorId) {
  ...
  const mentorStudent = await User.findOne({ _id: mentorId, role: 'mentor' })
    .select('_id')
    .lean();
...
function buildUpdatePayload(body, currentMaterial) {
  ...
  if ('visibility' in body) {
    ...
    if (!visibility) {
      throw new Error(`Visibility must be one of: ${MATERIAL_VISIBILITY.join(', ')}`);
    }
  ...
  if ('status' in body) {
    const status = normalizeStatus(body.status, currentMaterial.visibility);
...
export async function PUT(request, context) {
  try {
    ...
    const updates = buildUpdatePayload(body, currentMaterial);

    if (updates.subjectId) {
      await ensureSubjectAccess(updates.subjectId, currentUser.id);
    }
    ...
    const material = await Material.findOneAndUpdate(
      ...
      .populate({ path: 'mentorId', select: 'name email role' })
      .lean();
