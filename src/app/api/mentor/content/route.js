...
const databaseErrorPatterns = ['querySrv', 'ENOTFOUND', 'ECONNREFUSED', 'Mongo'];
...
function createErrorResponse(error) {
  ...
  if (databaseErrorPatterns.some((pattern) => errorMessage.includes(pattern))) {
  ...
  if (error instanceof Error) {
    ...
    if (error.message === 'Mentor not found' || error.message === 'Subject not found') {
...
async function buildCreatePayload(body, currentUser) {
  ...
  await ensureSubjectAccess(subjectId, mentorId);
...
export async function POST(request) {
  try {
    ...
    const populatedMaterial = await populateMaterial(material._id);
